...
function ieladejotPievienot(saites) {
    console.log("Starting ieladejotPievienot");

    for (let objekts = 1; objekts <= 8; objekts++) {
        let lieta = document.getElementById('img' + objekts);
        console.log("Checking: img" + objekts, lieta);
        if (lieta) {
            lieta.addEventListener('click', function () {
                console.log("Clicked: img" + objekts);
                inforadit(objekts);
            });
        }
    }
...
"<h4>Mācību vides/platformas</h4></br><ul><li> Interneta kopprojekti - projektu vienlaicīgi veic vairākas skolas</li><li>Skolu sadarbības tīkli - skolu sadarbības iespējas</li></ul>",
